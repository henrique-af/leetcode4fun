...
class Solution(object):
    ...
    def insertGreatestCommonDivisors(self, head):
        ...
        while head.next:
            ...
            )  # Calculate GCD of the current and next node values
            ...
            cur = (
                cur.next.next
            )  # Move to the last node in the new list (the newly added GCD node)

        cur.next = ListNode(
            head.val
        )  # After exiting the loop, add the last node's value

        return (
            ans.next
        )  # Return the new list starting from the first actual node (skipping the dummy node)